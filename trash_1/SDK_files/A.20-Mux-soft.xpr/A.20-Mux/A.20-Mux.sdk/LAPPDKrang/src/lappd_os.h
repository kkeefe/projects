...
typedef struct lappd {
  ...
  //
  //   ...
  //   short: desired register
  //   u8:    operation
  //          Bits:  MSB->LSB reserved(3) ack(1) silent(1) op(3)
  //                 ack: 0 = failure, 1 = great success
  //                  op: 0 = read, 1 = write, 2 = status, 3-6 = reserved, 7 = forbidden
  //              silent: 0 = echo back ack or nak, 1 = stay quiet
  //          NOTE: high bit 7 is never set.  This is because end of requests is 0xFF
  //          NOTE: ack is positive, because you want to see your frame echoed back at you with everything the same, except for success
  //   short: amount of data which follows.  (0 if reading or status)
  //
  //   -- DATABLOCK --
  //   ...
  //
  // End of requests: operation = 0xFF
  //
  u8 payload[0];
} lappd_t;
...
#define LAPPD_REG_MASK_SILENT 0x08   // 4-th bit position = 8
#define LAPPD_REG_MASK_OP 0x07  // bottom 3 bits
#define LAPPD_REG_MASK_ACK 0x10 // 5-th
#define LAPPD_REG_MASK_RESERVED 0xE0 // top 3 bits
